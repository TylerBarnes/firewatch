...
2. `fw view <pr>` gives a readable PR-level summary.
3. `fw list --pr <pr>` shows the feedback items in a readable format.
4. `fw --pr <pr> --type comment` is the structured fallback for custom filtering.

## Safe Defaults

- Prefer `--pr <number>` over `--mine` or `--reviews` until config is known-good.
- Prefer `fw view` and `fw list` when the user asks to "check in" on a PR or review.
...
2. `fw view <pr>` for readable context
3. `fw list --pr <pr>` for readable feedback enumeration
4. JSONL + `jq` only if the user wants filtering, counting, or extraction
